pipeline {
    ...
    environment {
        ...
        REACT_APP_GED_URL = "https://dev-ged.akieni.tech/backend/cnss"
    ...
    stages{
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    def IMAGE_TAG = "ims_frontend_dev-v${env.APP_VERSION}"
        ...
        stage('Deploy IMS Dev Frontend') {
            environment {
                ARGOCD_SERVER = credentials('argocd-server')
                ARGOCD_APP = "ims-frontend-dev"
            }
            steps {
